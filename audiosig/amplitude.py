...
from ._validation import validate_audio
...
def apply_gain_db(audio: np.ndarray, db: float, *, clip: bool = False) -> np.ndarray:
    ...
    source, _ = validate_audio(audio)
    gain_db = float(db)
    if np.isnan(gain_db) or gain_db == np.inf:
        raise InvalidParameterError("db must be finite or negative infinity")
    if not isinstance(clip, (bool, np.bool_)):
        raise InvalidParameterError("clip must be a boolean")
    if gain_db == -np.inf:
