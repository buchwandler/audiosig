# Changelog

> **Generated by releaseledger.** Manual edits may be overwritten. Update releaseledger entries and rebuild this file instead.

## [Unreleased]

<!-- releaseledger:unreleased-start version=0.1.4 -->

### Added

- Added BS.1770-style integrated loudness and dBFS/dBTP peak measurement APIs

### Documentation

- Documented loudness, peak, RMS, normalization, and downstream import boundaries

### Quality

- Improved loudness and true-peak conformance coverage
<!-- releaseledger:unreleased-end -->

## [0.1.3] - 2026-09-14

### Added

- Added deterministic NumPy-only smooth waveform cut-point selection within caller-provided sample intervals

### Documentation

- Documented smooth cut-point semantics, complexity, examples, and downstream policy boundaries

### Quality

- Improved boundary quality coverage across validation, downstream contracts, packaging, axes, and dtypes

<!-- generated by releaseledger -->

## [0.1.2] - 2026-08-01

### Added

- Added NumPy-only silence generation and explicit-axis mono downmixing APIs for TTSForge compatibility

<!-- generated by releaseledger -->

## [0.1.1] - 2026-08-01

### Added

- Added deterministic silence generation and explicit channel-axis arithmetic downmixing for portable TTS

<!-- generated by releaseledger -->

## [0.1.0] - 2026-07-31

### Added

- Phase-vocoder time stretching with configurable window size and overlap
- Windowed-sinc resampler with rounded output sizing and an optional librosa-compatible ceiling mode
- Pitch shifting via resample-then-stretch that preserves exact input length
- Normalized-energy and relative-dB voice activity detection algorithms
- RMS amplitude normalization and peak amplitude utilities
- Silence trimming and audio splitting with configurable frame parameters
- Audio framing, short-time energy, zero-crossing rate, and spectral flux utilities
- Typed AudioSignalError exceptions for invalid arrays and parameters
- NumPy-only dependency with no native extensions required
- NumPy-only WSOLA speech backend with explicit backend selection
- Experimental NumPy-only ESOLA speech backend with exact-length synthesis, epoch alignment, and explicit moderate-rate validation
- Experimental NumPy-only direct TD-PSOLA speech pitch/prosody backend with bounded pitch tracking, voiced-grain synthesis, and WSOLA unvoiced fallback
- Single-pass pitch/rate compositor planner and deterministic quality harness

### Documentation

- README overview plus detailed API, getting-started, advanced, and examples documentation
- Backend trade-offs, exact-length semantics, rolloff propagation, and native formant limitations
- ESOLA speech-only scope, rate limits, independent-lane behavior, and listening-evaluation gate
- TD-PSOLA experimental scope, conservative limits, fallback behavior, and real-speech listening-evaluation gate

### Quality

- Full test suite with quality, compatibility, and packaging tests
- Deterministic source-filter-style speech fixtures and local WAV comparison metrics

<!-- generated by releaseledger -->

<!-- releaseledger:generated-file -->
